...
<div class="page" id="p-org">
  ...
  function orgCard(node){
    ...
    return `<div style="display:flex;flex-direction:column;align-items:center;flex-shrink:0">
      <div class="section-item${node.color==='#ffffff'?' org-white-dots':''}" style="width:240px;flex-shrink:0;position:relative;cursor:pointer${node.color&&node.color!=='#ffffff'?`;background:${node.color}`:''}${node.color==='#ffffff'?';color:#16573a':''}" onclick="orgToggleDetail('${node.id}')">
  ...
  function orgToggleDetail(id){
    ...
    var html=`<div style="font-size:15px;font-weight:700;color:var(--white);margin-bottom:16px">${esc(node.name)}</div>
      <div style="display:flex;flex-direction:column;gap:10px;font-size:13px;margin-bottom:16px">
        ...
        <div><span class="flbl">Tel raqam</span><br><span style="color:var(--white-80)">${esc(node.tel||'—')}</span></div>
        <div><span class="flbl">Telegram</span><br><span style="color:var(--white-80)">${esc(node.tg||'—')}</span></div>
  ...
  function orgShowEditModal(id){
    ...
    var html=`<div style="font-size:15px;font-weight:700;color:var(--white);margin-bottom:16px">Tahrirlash</div>
      ...
      <div class="frow"><label class="flbl">Tel raqam</label><input class="fi" id="oem-tel" value="${esc(node.tel||'')}"></div>
      <div class="frow"><label class="flbl">Telegram</label><input class="fi" id="oem-tg" value="${esc(node.tg||'')}"></div>
      <div class="frow">
  ...
  function orgSaveEditModal(id){
    ...
    node.tel=document.getElementById('oem-tel').value.trim();
    node.tg=document.getElementById('oem-tg').value.trim();
...
function applyEntryCardPerms(){
  var map={'card-bolimlar':'bolimlar','card-org':'org','card-kaiten':'kaiten','card-hujjatlar':'hujjatlar','card-sekretar':'sekretar'};
...
function adminShowNewProfileForm(){
...
function adminPermsHtml(perms){
  ...
  var secHtml = sections.map(function(s){
    ...
    return '<label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#555;padding:4px 0 4px 18px;"><input type="checkbox" class="aperm-sec" value="'+escAdmin(s.id)+'" '+(checked?'checked':'')+'> '+escAdmin(s.title)+'</label>';
  }).join('');
...
function adminShowEditProfileForm(u){
  document.getElementById('admin-new-profile-form').innerHTML='';
...
function adminCard(u){
  ...
  return '<div style="background:white;border:1px solid #e2e8de;border-radius:12px;padding:14px;margin-bottom:10px;">'+
    ...
    '<div style="font-size:13px;color:#888;margin-bottom:10px;">@'+u.login+' · '+date+'</div>'+
    btns+'</div>';
}
...
function sekretarCard(it){
  ...
  var filesHtml=(it.files||[]).map(function(f){return `<a href="${esc(f.url)}" target="_blank" style="display:block;color:var(--lime);font-size:12px;margin-top:2px;text-decoration:none;word-break:break-all">📎 ${esc(f.name)}</a>`;}).join('');
  return `<div class="kaiten-card">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
      <div style="font-size:14px;font-weight:700;color:var(--lime)">📅 ${esc(sekretarFormatDateTime(it.date,it.time))}</div>
      <button class="abtn" onclick="sekretarFinish('${it.id}')" title="Yakunlash">☐ Yakunlash</button>
    </div>
    ${it.place?`<div style="font-size:13px;color:var(--white-80);margin-top:4px">📍 ${esc(it.place)}</div>`:''}
    ${person?`<div style="font-size:13px;color:var(--white-80);margin-top:2px">👤 ${esc(person)}</div>`:''}
    ${it.purpose?`<div class="kaiten-card-text" style="margin-top:4px">🎯 ${esc(it.purpose)}</div>`:''}
    ${it.link?`<a href="${esc(it.link)}" target="_blank" style="display:block;color:var(--lime);font-size:12px;margin-top:4px;word-break:break-all;text-decoration:none">🔗 ${esc(it.link)}</a>`:''}
    ${filesHtml}
    ${it.note?`<div style="font-size:12px;color:var(--white-50);margin-top:4px">📝 ${esc(it.note)}</div>`:''}
    <div class="kaiten-card-btns">
      <button class="abtn" onclick="showSekretarForm('${it.id}')">✏</button>
      <button class="abtn abtn-red" onclick="sekretarDelete('${it.id}')">✕</button>
    </div>
  </div>`;
}
...
function sekretarArchiveCard(it){
  var person=[it.personName,it.personRole,it.personPhone].filter(Boolean).join(' — ');
  var filesHtml=(it.files||[]).map(function(f){return `<a href="${esc(f.url)}" target="_blank" style="display:block;color:var(--lime);font-size:12px;margin-top:2px;text-decoration:none;word-break:break-all">📎 ${esc(f.name)}</a>`;}).join('');
  return `<div class="kaiten-card" style="opacity:.75">
    <div style="font-size:14px;font-weight:700;color:var(--white-50)">✅ 📅 ${esc(sekretarFormatDateTime(it.date,it.time))}</div>
    ${it.place?`<div style="font-size:13px;color:var(--white-80);margin-top:4px">📍 ${esc(it.place)}</div>`:''}
    ${person?`<div style="font-size:13px;color:var(--white-80);margin-top:2px">👤 ${esc(person)}</div>`:''}
    ${it.purpose?`<div class="kaiten-card-text" style="margin-top:4px">🎯 ${esc(it.purpose)}</div>`:''}
    ${it.link?`<a href="${esc(it.link)}" target="_blank" style="display:block;color:var(--lime);font-size:12px;margin-top:4px;word-break:break-all;text-decoration:none">🔗 ${esc(it.link)}</a>`:''}
    ...
    ${it.note?`<div style="font-size:12px;color:var(--white-50);margin-top:4px">📝 ${esc(it.note)}</div>`:''}
    <div class="kaiten-card-btns">
      <button class="abtn abtn-red" onclick="sekretarDeleteFromArchive('${it.id}')">✕</button>
    </div>
  </div>`;
}
...
function kaitenGetEmployees(deptName){
  var dept = orgData.find(n=>kaitenNorm(n.name)===kaitenNorm(deptName));
  if(!dept) return [];
  return orgData.filter(n=>n.parentId===dept.id && n.fio && n.fio.trim());
}
function kaitenGetAllEmployees(){
  var result=[];
  KAITEN_DEPTS.forEach(function(deptName){
    var dept=orgData.find(function(n){return kaitenNorm(n.name)===kaitenNorm(deptName);});
    if(!dept) return;
    orgData.filter(function(n){return n.parentId===dept.id && n.fio && n.fio.trim();}).forEach(function(e){
      result.push({fio:e.fio, role:e.name, dept:deptName, tg:e.tg||''});
    });
  });
  return result;
}
...
function kaitenUpdateEmpOptions(prefix, presetVal){
  ...
  if(!dept){ empSel.innerHTML = '<option value="">— avval bo&#39;limni tanlang —</option>'; return; }
  var emps = kaitenGetEmployees(dept);
  ...
  empSel.innerHTML = '<option value="">— tanlang —</option>' + emps.map(function(e){
    var val = e.fio+'|'+e.name+'|'+(e.tg||'');
    var sel = (presetVal && presetVal.split('|').slice(0,2).join('|')===val.split('|').slice(0,2).join('|')) ? 'selected' : '';
    return `<option value="${esc(val)}" ${sel}>${esc(e.fio)} (${esc(e.name)})</option>`;
  }).join('');
}
...
function kaitenMove(id, dir){
  ...
  kaitenSave(); renderKaiten();
  kaitenNotifyMove(t, colTitle);
}
function kaitenNotifyMove(t, colTitle){
  try{
    var lines=[
      '📋 Vazifa "'+colTitle+'" ustuniga o\'tkazildi',
...
function kaitenCard(t){
  ...
  return `<div class="kaiten-card">
    ...
    <div class="kaiten-card-btns">
      ...
      <button class="abtn" style="${(canLeft||canRight)?'':'margin-right:auto'}" onclick="kaitenToggleChat('${t.id}')">💬 ${comments.length}</button>
